...
$CC $CFLAGS $(pkg-config --cflags gtk4-layer-shell-0) $(pkg-config --cflags gtk4) -o bow src/main.c src/window.c src/log.c src/config.c $(pkg-config --libs gtk4-layer-shell-0) $(pkg-config --libs gtk4)
