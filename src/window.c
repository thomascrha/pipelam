...
static void bow_render_window(GtkApplication *app, gpointer data) {
    ...
    bow_log_debug("len of volume_expression: %lu", strlen(((struct data *)data)->volume_expression));

    // combine markup and border
    gtk_label_set_markup(GTK_LABEL(label), ((struct data *)data)->volume_expression);
    gtk_window_set_child(gtk_window, label);

    gtk_window_present(gtk_window);

    bow_log_debug("window_timeout: %d", ((struct data *)data)->window_timeout);
    g_timeout_add(((struct data *)data)->window_timeout, (GSourceFunc)gtk_window_close, gtk_window);
}

int bow_create_run_window(char *volume_expression, int window_timeout) {
    bow_log_debug("Opening window with volume expression: %s", volume_expression);
