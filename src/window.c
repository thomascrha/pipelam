...
static void
bow_render_window (GtkApplication *app, gpointer data)
{
    ...
    bow_log_debug ("window_timeout: %d",
                   ((struct data *)data)->window_timeout);
    g_timeout_add (((struct data *)data)->window_timeout,
                   (GSourceFunc)gtk_window_close, gtk_window);
}

int
bow_create_run_window (char *volume_expression, int window_timeout)
{
    bow_log_info ("Received string: %s", volume_expression);
    GtkApplication *app = gtk_application_new ("com.github.wmww.bow",
